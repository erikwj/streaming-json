...
abstract class JmhBenchmarks(name: String) {
  ...
  @Benchmark
   def playJawnParse() =
     jawn.support.play.Parser.parseFromFile(new File(path)).get
...
//class Ugh100kBench extends JmhBenchmarks("big100k.json")
class ESBench extends JmhBenchmarks("ES_result.json")
